        ...
        public static void InitializeOnLoad()
        {
            if (!EditorPrefs.HasKey(BUNDLE_BEST_PRACTICE_ALERT_SHOWN))
            {
                    ...
                    || iosBundleId.Any(char.IsUpper))
                {
                    ...
                    int updateBundleIds = EditorUtility.DisplayDialogComplex("Homa Belly Best Pracite Tip", string.Format("Your Android & iOS application identifiers differ or have upper case characters:" +
                        "\n\n• Android: {0}\n• iOS: {1}\n\n" +
                        "It is recommended to be the same for both platforms and use only lower case characters.\n\n" +
                        "Do you want to unfiy them now?", androidBundleId, iosBundleId), "Use Android", "Use iOS", "Will do it manually");
                    ...
                    switch(updateBundleIds)
                    {
                        ...
                        case 0:
                            ...
                            EditorUtility.DisplayDialog("Homa Belly Best Practice Tip", string.Format("Application identifiers updated to: {0}", androidBundleId.ToLower()), "Ok");
                            break;
                        // Use iOS
                        case 1:
                            ...
                            EditorUtility.DisplayDialog("Homa Belly Best Practice Tip", string.Format("Application identifiers updated to: {0}", iosBundleId.ToLower()), "Ok");
                            break;
                        // Do it manually
                        case 2:
