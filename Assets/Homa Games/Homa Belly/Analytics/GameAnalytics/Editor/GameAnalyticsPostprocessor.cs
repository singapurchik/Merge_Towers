...
namespace HomaGames.HomaBelly
{
    public class GameAnalyticsPostprocessor : AssetPostprocessor
    {
        ...
        static void Configure()
        {
            ...
            // When project is recompiled from scratch, the GA settings are not loaded properly and we end up overriding some properties of the
            // configuration so we don't update the settings if they don't exist
            if (UnityEditorInternal.InternalEditorUtility.inBatchMode
               || !AssetDatabase.LoadAssetAtPath<GameAnalyticsSDK.Setup.Settings>(SETTINGS_PATH))
            {
                return;
            }
        ...
        {
            if (configurationData.ContainsKey("b_configure_custom_dimensions"))
            {
                bool configureCustomDimensions = true;
                bool.TryParse(configurationData["b_configure_custom_dimensions"], out configureCustomDimensions);

                // If remote configuration explicitly ask for not to configure, skip 
                if (!configureCustomDimensions)
                {
                    HomaGamesLog.Debug("Skipping Custom Dimensions configuration due to explict manifest request. See `b_configure_custom_dimensions`");
                    return;
                }
            }
            ...
            // Avoid annoy the dev during 12 hours on each compilation
            if (!ShouldCheckForCustomDimensionsConfiguration())
            {
                return;
            }
            ...
            // Determine if update is required, so ask the dev for allowance
            if (ShouldUpdateCustomDimensions(GameAnalytics.SettingsGA.CustomDimensions01, configurationCustomDimensions01)
                || ShouldUpdateCustomDimensions(GameAnalytics.SettingsGA.CustomDimensions02, configurationCustomDimensions02)
                || ShouldUpdateCustomDimensions(GameAnalytics.SettingsGA.CustomDimensions03, configurationCustomDimensions03))
            {
                // Custom dimensions need to be updated
                bool accepted = EditorUtility.DisplayDialog("Game Analytics Custom Dimensions",
                    "We detected you are using Homa Games NTesting and Game Analytics. For a proper and automatic integration," +
                    "we need to update your current Custom Dimensions configuration with NTesting values. Do you allow us to proceed?", "Accept", "Hold on. I will ask my PM first");

                // If accepted, update custom dimensions
                if (accepted)
                {
            ...
            {
                HomaGamesLog.Debug("Game Analytics Custom Dimensions already properly configured for NTesting");
            }
        }

        /// <summary>
        /// Determine if the last time we asked for configuration was before HOURS_UNTIL_NEXT_CUSTOM_DIMENSIONS_CHECK.
        /// This will avoid annoying the developer with constant popups.
        /// </summary>
        /// <returns>True if configuration acceptande should be done/requested. False otherwise</returns>
        private static bool ShouldCheckForCustomDimensionsConfiguration()
        {
            bool shouldConfigure = true;
            DateTime now = DateTime.Now;
            if (EditorPrefs.HasKey(LAST_CUSTOM_DIMENSIONS_CONFIGURATION_CHECK_TIMESTAMP_KEY))
            {
                long lastCheckFileTime;
                long.TryParse(EditorPrefs.GetString(LAST_CUSTOM_DIMENSIONS_CONFIGURATION_CHECK_TIMESTAMP_KEY), out lastCheckFileTime);

                // Check new version after HOURS_UNTIL_NEXT_CUSTOM_DIMENSIONS_CHECK hours of last check
                TimeSpan deltaTimeSpan = now.Subtract(DateTime.FromFileTime(lastCheckFileTime));
                if (deltaTimeSpan.TotalHours < HOURS_UNTIL_NEXT_CUSTOM_DIMENSIONS_CHECK)
                {
            ...
            {
                // Set first time check as `now`
                EditorPrefs.SetString(LAST_CUSTOM_DIMENSIONS_CONFIGURATION_CHECK_TIMESTAMP_KEY, now.ToFileTime().ToString());
            }
